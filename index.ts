...
server.use(helmet());
server.get("/", (req, res) =>
  res.status(200).send("The Lost Battlion lives once again!")
);
server.listen(80, () => {});
/* -----DISCORD BOT----- */
...
sql
  .prepare(
    `CREATE TABLE IF NOT EXISTS TheLostBattlion (userid TEXT PRIMARY KEY UNIQUE, strikes TEXT, strikeNumber TEXT DEFAULT 0);`
  )
  .run();
...
const strikeClearer = new cron.CronJob("* * * * *", () => {
  ...
  strikelogs.forEach(async (strikelog) => {
    const strikes: { reason: string; tilldate: string; messageid: string }[] =
      JSON.parse(strikelog.strikes);
    strikes.forEach(async (strike) => {
      if (moment().unix() >= parseInt(strike.tilldate)) {
        ...
        if (strikeChannel?.isTextBased()) {
          const strikeMessage = (await strikeChannel.messages.fetch()).find(
            (msg) => msg.id === strike.messageid
          );
          if (!strikeMessage) return;
          const strikeEmbed = JSON.parse(
            JSON.stringify(strikeMessage?.embeds[0])
          );
          ...
          const updatedStrikelog: {
            ...
            ...{
              strikes: JSON.stringify(
                strikes.filter((v) => moment().unix() < parseInt(v.tilldate))
              ),
          ...
          sql
            ...
            .run(
              updatedStrikelog.userid,
              updatedStrikelog.strikes,
              updatedStrikelog.strikeNumber
            );
...
const recruitMemberCommand = new Discord.SlashCommandBuilder()
  ...
  .addStringOption((input) =>
    input
      .setName("ign")
      .setDescription(
        "Provide the In-Game name of the recruited. (Format: FirstName_LastName)"
      )
      .setRequired(true)
  )
  .addAttachmentOption((attachment) =>
    attachment
      .setName("evidence")
      .setDescription(
        "Attach a screenshot of the recruitment evidence. (Note: A screenshot of the recruitment process)"
      )
      .setRequired(true)
  )
  .addBooleanOption((bool) =>
    bool
      .setName("invitation")
      .setDescription(
        "Is the recruited invited to The Lost Battalion discord server?"
      )
      .setRequired(true)
  )
  .addStringOption((comments) =>
    comments
...
const blacklistCommand = new Discord.SlashCommandBuilder()
  ...
  .addStringOption((user) =>
    user
      .setName("member")
      .setDescription(
        "Name of the member you want to blacklist. (Format: FirstName_LastName)"
      )
      .setRequired(true)
  )
  .addStringOption((input) =>
  ...
  if (interaction.isButton()) {
    ...
    if (
      !interactionMember?.roles.cache.find(
        (role) => role.id === ("1139667002097664191" || "1139667184839303328")
      )
    )
      return;
    ...
    if (interaction.customId === "strike-forgive") {
      if (
        strikedEmbed.footer?.text.split("⚬")[0].trim() ===
      ...
      const strikeMember = (await interaction.guild?.members.fetch())?.get(
        strikeMemberId
      ...
      const memberStrikeLog: {
        ...
        (strikelog) => `${strikelog.userid}` === strikeMemberId
      ) as {
        userid: string;
        strikes: string;
        strikeNumber: string;
      ...
      const updatedStrikes = memberStrikes.filter(
        (strike) => strike.messageid !== interaction.message.id
      );
      ...
      const strikeEmbed = new Discord.EmbedBuilder(
        interaction.message.embeds[0] as Discord.EmbedData
      );
      strikeEmbed
        .setDescription(
          `${strikeEmbed.data.description}\n\n**\`You have been forgiven by\`**<@${interaction.user.id}>`
        )
        .setFooter({
        ...
        await interaction.reply({
          embeds: [
            new Discord.EmbedBuilder()
              .setTitle("Strike Log")
              .setDescription(
                `You have deleted <@${strikeMember?.id}>'s strike log`
              )
              .setTimestamp()
              .setColor(Color("#ff5733").rgbNumber()),
          ],
  ...
  if (interaction.isChatInputCommand()) {
    if (interaction.commandName === "strike") {
      ...
      if (
        ...
        (interactionMember?.roles?.highest.rawPosition as number) <=
        ...
        await interaction.editReply({
          embeds: [
            new Discord.EmbedBuilder()
              .setTitle(`Strike Logs`)
              .setDescription(
                `You cannot strike members higher or equal to your rank`
              )
              .setTimestamp()
              .setColor(Color("#ff5733").rgbNumber()),
          ],
        ...
        if (currentStrikeNumber === 3) {
          ...
          strikeComponents.setComponents(
            forgiveButton,
            kickButton,
            banButton,
            deleteButton
        ...
        if (strikeChannel?.isTextBased())
          strikeChannel
            ...
            .then(async (message) => {
              ...
              sql
                .prepare(
                  `INSERT OR REPLACE INTO TheLostBattlion VALUES(?, ?, ?)`
                )
                .run(
                  member.id,
                  JSON.stringify(memberStrikes),
                  currentStrikeNumber
                );
              await interaction
                .editReply({
                  embeds: [
                    new Discord.EmbedBuilder()
                      .setTitle(`Strike Logs`)
                      .setDescription(
                        `Your strike has been added ${message.url}`
                      )
                      .setColor(Color("#ff5733").rgbNumber()),
                  ],
                })
      ...
      if (
        ...
        const recruitmentEmbed = new Discord.EmbedBuilder()
          .setTitle("Recruitment logs")
          .setDescription(
            "A new member has been recruited to The Lost Battlion"
          )
          .setFields([
            {
              name: "Recruiter",
              ...
              await interaction.editReply({
                embeds: [
                  new Discord.EmbedBuilder()
                    .setTitle("Recruitment logs")
                    .setDescription(
                      `Your recruitment has been added to the logs. ${message.url}`
                    )
                    .setColor(Color("#ff5733").rgbNumber())
                    .setTimestamp(),
                ],
            ...
            await interaction.editReply({
              embeds: [
                new Discord.EmbedBuilder()
                  .setTitle("Blacklist Logs")
                  .setDescription(
                    `Member has been blacklisted and added to the logs. ${message.url}`
                  )
                  .setColor(Color("#ff5733").rgbNumber())
                  .setTimestamp(),
              ],
