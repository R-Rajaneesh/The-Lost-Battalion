...
server.use(cors());
server.use(helmet());
server.get("/", (req, res) => res.status(200).send("Kilo Tray Ballas lives once again!"));
server.listen(80, () => {});
/* -----DISCORD BOT----- */
...
const client = new Discord.Client({
	...
	presence: {
		...
		activities: [
			{
				name: `Kilo Tray Ballas lives once again!`,
...
const strikeClearer = new cron.CronJob("* * * * *", () => {
	const strikelogs: {
		...
		strikes: string;
		strikeNumber: string;
	}[] = sql.prepare(`SELECT * FROM TheLostBattlion;`).all() as {
		userid: string;
		strikes: string;
		strikeNumber: string;
	}[];
	strikelogs.forEach(async (strikelog) => {
		const strikes: { reason: string; tilldate: string; messageid: string }[] = JSON.parse(strikelog.strikes);
...
const recruitMemberCommand = new Discord.SlashCommandBuilder()
	...
	.addBooleanOption((bool) => bool.setName("invitation").setDescription("Is the recruited invited to Kilo Tray Ballas discord server?").setRequired(true))
	.addStringOption((comments) => comments.setName("additional-comments").setDescription("Provide a comment for the recruitment.").setRequired(false));
...
/* -----EVENTS----- */
client.once("ready", () => {
	rest.put(Routes.applicationCommands(process?.env?.BOT_ID as string), {
		body: [strikeMemberCommand, recruitMemberCommand, blacklistCommand],
	});
	console.log("Kilo Tray Ballas is living!");
	strikeClearer.start();
});
...
client.on("interactionCreate", async (interaction) => {
	if (interaction.isButton()) {
		...
		if (!interactionMember?.roles.cache.find((role) => role.id === ("1139667002097664191" || "1139667184839303328"))) return;

		const strikedEmbed = interaction.message.embeds[0] as Discord.EmbedData;

		if (interaction.customId === "strike-forgive") {
			if (strikedEmbed.footer?.text.split("⚬")[0].trim() === ("Forgiven" || "Kicked" || "Banned")) return;
			const strikeMemberId = strikedEmbed?.footer?.text.split("⚬")[1].trim() as string;

			const strikeMember = (await interaction.guild?.members.fetch())?.get(strikeMemberId) as Discord.GuildMember;
			const strikelogs: {
				userid: string;
				strikes: string;
				strikeNumber: string;
			...
			const memberStrikeLog: {
				userid: string;
				strikes: string;
				strikeNumber: string;
			} = strikelogs.find((strikelog) => `${strikelog.userid}` === strikeMemberId) as {
				userid: string;
				strikes: string;
				strikeNumber: string;
			};
			...
			const memberStrikes: {
				...
				tilldate: string;
				messageid: string;
			}[] = JSON.parse(memberStrikeLog?.strikes as string);
			...
			const strikeEmbed = new Discord.EmbedBuilder(interaction.message.embeds[0] as Discord.EmbedData);
			...
			if (strikedEmbed.footer?.text === ("Forgiven" || "Kicked" || "Banned")) return;
			const strikeMemberId = strikedEmbed?.footer?.text.split("⚬")[1].trim() as string;
			...
			const strikeMember = (await interaction.guild?.members.fetch())?.get(strikeMemberId) as Discord.GuildMember;
			if (!strikedEmbed?.footer?.text) await interaction.message.delete();
			else {
				const strikelogs: {
					userid: string;
					strikes: string;
	...
	if (interaction.isChatInputCommand()) {
		if (interaction.commandName === "strike") {
			...
			if (!interactionMember?.roles.cache.find((role) => role.id === "1139667002097664191" || role.id === "1139667184839303328")) {
			...
			} else if ((interactionMember?.roles?.highest.rawPosition as number) <= member?.roles?.highest?.rawPosition) {
				await interaction.editReply({
					embeds: [
						new Discord.EmbedBuilder()
							.setTitle(`Strike Logs`)
				...
				const memberData: {
					...
					strikes: string;
					strikeNumber: string;
				} = sql.prepare(`SELECT * FROM TheLostBattlion WHERE userid = (?)`).get(member.id) as {
					userid: string;
					strikes: string;
					strikeNumber: string;
				};
				...
				const strikeComponents = new Discord.ActionRowBuilder<Discord.ButtonBuilder>();
			...
			if (
				!interactionMember?.roles.cache.find(
					(role) => role.id === "1139667002097664191" || role.id === "1139667184839303328" || role.id === "1139667424971599934",
				)
			) {
				await interaction.editReply({
					embeds: [
				...
				const recruitmentEmbed = new Discord.EmbedBuilder()
					.setTitle("Recruitment logs")
					.setDescription("A new member has been recruited to Kilo Tray Ballas")
					.setFields([
						{
							name: "Recruiter",
							value: `<@${interaction.user.id}>`,
					...
					.setImage(evidence?.url as string)
					.setColor(Color("#ff5733").rgbNumber())
					.setTimestamp();
			...
			if (!interaction.inGuild()) return;
			await interaction.deferReply({ ephemeral: true });
			const user = interaction.options.getString("member") as string;
			...
			const blacklistComponents = new Discord.ActionRowBuilder<Discord.ButtonBuilder>().addComponents(deleteButton);
			const blacklistEmbed = new Discord.EmbedBuilder()
				.setTitle("Blacklist Logs")
				.setDescription(`\`${user}\` is blacklisted in Kilo Tray Ballas.`)
				.setFields([
					{
						name: "Blacklister",
						value: `<@${interaction.user.id}>`,
...
client.on("messageReactionAdd", async (reaction, user) => {
	...
	if (reaction.emoji.name === "🗑️" && reaction?.message?.author?.id === client?.user?.id && reaction.message.embeds) {
		if (reaction.message.embeds[0].fields[0].name === "Recruiter") {
			if (reaction.message.embeds[0].fields[0].value.replace("<@", "").replace(">", "") === user.id) {
